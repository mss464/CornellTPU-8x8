...
class TpuRtlDriver:
    ...
    async def write_bram(self, addr, values):
        ...
        await self.write_axi_lite(0x10, addr)          # addr_devmem
        await self.write_axi_lite(0x18, len(values))   # length
    ...
    async def read_bram(self, addr, length):
        ...
        await self.wait_for_flag(0x04, 1)  # instr_ready
        await self.write_axi_lite(0x10, addr)     # addr_devmem
        await self.write_axi_lite(0x18, length)   # length
        ...
        # Unpack received bytes back to float32 list
        raw = bytes(frame.tdata)
        n_words = len(raw) // 4
        return [int_to_float(struct.unpack_from('<I', raw, i * 4)[0])
