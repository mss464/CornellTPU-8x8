...
# We added workflow/kernels to sys.path in tinytorch/numpy/custom_kernels.py
# but here we can just import it from the folder
sys.path.append(os.path.join(str(project_root), "workflow", "kernels"))
try:
    import systolic_tiled_matmul
except ImportError:
    print("Error: Could not find systolic_tiled_matmul in workflow/kernels/")
    sys.exit(1)
