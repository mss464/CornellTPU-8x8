# Verification: fp32_mul.sv

## Scope

This document covers verification coverage for `src/compute_tile/fp32_mul.sv`, extracted from the master verification document.

---

## Role

IEEE-754 single-precision FP32 multiplier. Purely combinational block used by `pe.sv` (MAC) and `vpu_op.sv` (per-lane operations).
...
- Normal FP32 multiplication across a range of magnitudes and signs
...
- Overflow (result exceeds FP32 max) not tested at `fp32_mul` unit level
